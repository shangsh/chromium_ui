...
#include "../layout/standard_layout.h"
...
namespace view
{
    ...
    namespace
    {
        ...
        void UpdateButtonHelper(NativeButton* button_view,
            DialogDelegate* delegate, MessageBoxFlags::DialogButton button)
        {
        ...
        {
            canvas->FillRectInt(gfx::Color(color), 0, 0, view->width(), view->height());
        }
    ...
    void DialogClientView::ShowDialogButtons()
    {
        ...
        if(buttons&MessageBoxFlags::DIALOGBUTTON_OK && !ok_button_)
        {
            ...
            ok_button_ = new DialogButton(this, window(),
                MessageBoxFlags::DIALOGBUTTON_OK, label, is_default_button);
            ...
            {
                ok_button_->AddAccelerator(Accelerator(VKEY_ESCAPE,
                    false, false, false));
            }
        ...
        if(buttons&MessageBoxFlags::DIALOGBUTTON_CANCEL && !cancel_button_)
        {
            ...
            bool is_default_button =
                (dd->GetDefaultDialogButton()&MessageBoxFlags::DIALOGBUTTON_CANCEL) != 0;
            cancel_button_ = new DialogButton(this, window(),
                MessageBoxFlags::DIALOGBUTTON_CANCEL,
                label, is_default_button);
            cancel_button_->SetGroup(kButtonGroup);
            cancel_button_->AddAccelerator(Accelerator(VKEY_ESCAPE,
                false, false, false));
            if(is_default_button)
            {
                default_button_ = ok_button_;
        ...
        if(!buttons)
        {
            ...
            AddAccelerator(Accelerator(VKEY_ESCAPE, false, false, false));
        }
    }
        ...
        View* focused_now)
    {
        ...
        else
        {
            ...
            if((dd->GetDefaultDialogButton()&MessageBoxFlags::DIALOGBUTTON_CANCEL) != 0)
            {
    ...
    gfx::Size DialogClientView::GetPreferredSize()
    {
        ...
        if(has_dialog_buttons())
        {
            ...
            if(width > 0)
            {
                ...
                prefsize.set_width(__max(prefsize.width(), width));
            }
        }
        if(bottom_view_)
        {
    ...
    bool DialogClientView::AcceleratorPressed(const Accelerator& accelerator)
    {
        ...
        DCHECK(accelerator.GetKeyCode() == VKEY_ESCAPE);
        Close();
        return true;
    }
    ...
    void DialogClientView::ButtonPressed(Button* sender, const view::Event& event)
    {
    ...
    int DialogClientView::GetButtonWidth(int button) const
    {
        ...
        //return __max(string_width+kDialogButtonLabelSpacing,
    ...
    void DialogClientView::LayoutDialogButtons()
    {
        ...
        if(cancel_button_)
        {
            ...
            extra_bounds.set_width(__max(0, cancel_button_->x()));
            extra_bounds.set_y(cancel_button_->y());
            button_height = __max(button_height, ps.height());
        }
        if(ok_button_)
        {
            ...
            ok_button_->SetBounds(button_x, button_y, ok_button_right-button_x,
                ps.height());
            // The extra view bounds are dependent on this button.
            extra_bounds.set_width(__max(0, ok_button_->x()));
        ...
        if(extra_view_)
        {
            ...
            int height = size_extra_view_height_to_buttons_ ?
                __max(ps.height(), button_height) : ps.height();
    ...
    {
        gfx::Rect lb = GetLocalBounds(false);
        lb.set_height(__max(0, lb.height() - GetButtonsHeight()));
