...
namespace view
{
    ...
    // static
    Window* Window::CreateWanWindow(HWND parent,
    ...
    void WindowWin::SetWindowBounds(const gfx::Rect& bounds,
        HWND other_window)
    {
        SetChildBounds(GetNativeView(), GetParent(), other_window,
    ...
    void WindowWin::FrameTypeChanged()
    {
        if(base::GetWinVersion() >= base::WINVERSION_VISTA)
        {
            ...
            DwmapiWrapper::DWMNCRENDERINGPOLICY policy =
                non_client_view_->UseNativeFrame() ? DwmapiWrapper::DWMNCRP_ENABLED
                : DwmapiWrapper::DWMNCRP_DISABLED;
            DwmapiWrapper::instance()->DwmSetWindowAttribute(GetNativeView(),
                DwmapiWrapper::DWMWA_NCRENDERING_POLICY,
                &policy,
    ...
    void WindowWin::UpdateWindowTitle()
    {
        ...
        if(base::AdjustStringForLocaleDirection(window_title, &localized_text))
        {
    ...
    HWND WindowWin::GetNativeWindow() const
    {
    ...
    void WindowWin::Init(HWND parent, const gfx::Rect& bounds)
    {
        ...
        base::SetWindowUserData(GetNativeView(), this);
    ...
    {
        CenterAndSizeWindow(owning_window(), GetNativeView(),
            non_client_view_->GetPreferredSize().ToSIZE(), false);
    }
    ...
    void WindowWin::OnActivateApp(BOOL active, DWORD thread_id)
    {
        if(!active && thread_id!=GetCurrentThreadId())
        {
            ...
            // Update the native frame too, since it could be rendering the 
            // non-client area.
    ...
    void WindowWin::OnMouseLeave()
    {
        ...
        if(GetCursorPos(&pt))
        {
            LRESULT ht_component = ::SendMessage(GetNativeView(),
                WM_NCHITTEST, 0, MAKELPARAM(pt.x, pt.y));
    ...
    LRESULT WindowWin::OnNCHitTest(const gfx::Point& point)
    {
    ...
    void WindowWin::OnNCPaint(HRGN rgn)
    {
        ...
        if((window_rect.right-window_rect.left)!=root_view_->width() ||
            (window_rect.bottom-window_rect.top)!=root_view_->height())
        {
    ...
    void WindowWin::OnNCLButtonDown(UINT ht_component, const gfx::Point& point)
    {
    ...
    void WindowWin::OnNCRButtonDown(UINT ht_component, const gfx::Point& point)
    {
    ...
    void WindowWin::OnRButtonUp(UINT ht_component, const gfx::Point& point)
    {
        if(is_right_mouse_pressed_on_caption_)
        {
            ...
            POINT screen_point = point.ToPOINT();
    ...
    void WindowWin::OnSettingChange(UINT flags, const wchar_t* section)
    {
        if(!GetParent() && (flags==SPI_SETWORKAREA))
        {
            ...
            ::SetWindowPos(GetNativeView(), 0, 0, 0, 0, 0,
    ...
    void WindowWin::OnSize(UINT size_param, const gfx::Size& new_size)
    {
    ...
    void WindowWin::OnSysCommand(UINT notification_code, gfx::Point click)
    {
    ...
    void WindowWin::UpdateAccessibleName(std::wstring& accessible_name)
    {
        ...
        if(SUCCEEDED(hr))
        {
            base::ScopedVariant var(accessible_name.c_str());
    ...
    void WindowWin::ProcessNCMousePress(const gfx::Point& point, int flags)
    {
        POINT temp = point.ToPOINT();
        MapWindowPoints(HWND_DESKTOP, GetNativeView(), &temp, 1);
        ...
        message_flags |= flags;
        ProcessMousePressed(gfx::Point(temp), message_flags, false, false);
    }
