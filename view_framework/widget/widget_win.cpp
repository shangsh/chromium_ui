...
#define OBJID_CUSTOM 1
    ...
    WidgetWin* WidgetWin::GetWidget(HWND hwnd)
    {
        ...
        return reinterpret_cast<WidgetWin*>(base::GetWindowUserData(hwnd));
    }
    ...
    void WidgetWin::PaintNow(const gfx::Rect& update_rect)
    {
        ...
        else if(root_view_->NeedsPainting(false) && IsWindow())
        {
            if(!opaque_ && GetParent())
            {
                ...
                RECT parent_update_rect = update_rect.ToRECT();
                POINT location_in_parent = { 0, 0 };
                ClientToScreen(hwnd(), &location_in_parent);
                ScreenToClient(GetParent(), &location_in_parent);
                ::OffsetRect(&parent_update_rect, location_in_parent.x, location_in_parent.y);
    ...
    {
        return view::IsWindowActive(hwnd());
    }
        ...
        const gfx::Point& point)
    {
        ...
        ProcessMousePressed(point_in_widget, MK_LBUTTON, false, false);
    }
    ...
    bool WidgetWin::ContainsNativeView(HWND native_view)
    {
        if(hwnd() == native_view)
        {
            return true;
    ...
    {
        KeyEvent event(Event::ET_KEY_PRESSED, KeyboardCodeForWindowsKeyCode(c),
    ...
    {
        KeyEvent event(Event::ET_KEY_RELEASED, KeyboardCodeForWindowsKeyCode(c),
            KeyEvent::GetKeyStateFlags(), rep_cnt, flags);
        RootView* root_view = GetFocusedViewRootView();
        if(!root_view)
        {
    ...
    void WidgetWin::OnKillFocus(HWND focused_window)
    {
        GetFocusManager()->GetWidgetFocusManager()->OnWidgetFocusEvent(
            this->GetNativeView(), focused_window);
        SetMsgHandled(FALSE);
    }
    ...
    void WidgetWin::OnLButtonDown(UINT flags, const gfx::Point& point)
    {
        ProcessMousePressed(point, flags|MK_LBUTTON, false, false);
    }

    void WidgetWin::OnLButtonUp(UINT flags, const gfx::Point& point)
    {
        ProcessMouseReleased(point, flags|MK_LBUTTON);
    }

    void WidgetWin::OnLButtonDblClk(UINT flags, const gfx::Point& point)
    {
        ProcessMousePressed(point, flags|MK_LBUTTON, true, false);
    }

    void WidgetWin::OnMButtonDown(UINT flags, const gfx::Point& point)
    {
        ProcessMousePressed(point, flags|MK_MBUTTON, false, false);
    }

    void WidgetWin::OnMButtonUp(UINT flags, const gfx::Point& point)
    {
        ProcessMouseReleased(point, flags|MK_MBUTTON);
    }

    void WidgetWin::OnMButtonDblClk(UINT flags, const gfx::Point& point)
    {
    ...
    void WidgetWin::OnMouseMove(UINT flags, const gfx::Point& point)
    {
    ...
    LRESULT WidgetWin::OnMouseWheel(UINT message, WPARAM w_param, LPARAM l_param)
    {
        ...
        if(message==WM_MOUSEWHEEL &&
            view::RerouteMouseWheel(hwnd(), w_param, l_param))
        {
            return 0;
        }
    ...
    void WidgetWin::OnNCLButtonDblClk(UINT flags, const gfx::Point& point)
    {
        SetMsgHandled(ProcessMousePressed(point, flags|MK_LBUTTON, true, true));
    }

    void WidgetWin::OnNCLButtonDown(UINT flags, const gfx::Point& point)
    {
        SetMsgHandled(ProcessMousePressed(point, flags|MK_LBUTTON, false, true));
    }

    void WidgetWin::OnNCLButtonUp(UINT flags, const gfx::Point& point)
    {
        SetMsgHandled(FALSE);
    }

    void WidgetWin::OnNCMButtonDblClk(UINT flags, const gfx::Point& point)
    {
        SetMsgHandled(ProcessMousePressed(point, flags | MK_MBUTTON, true, true));
    }

    void WidgetWin::OnNCMButtonDown(UINT flags, const gfx::Point& point)
    {
        SetMsgHandled(ProcessMousePressed(point, flags | MK_MBUTTON, false, true));
    }

    void WidgetWin::OnNCMButtonUp(UINT flags, const gfx::Point& point)
    {
        SetMsgHandled(FALSE);
    }
    ...
    LRESULT WidgetWin::OnNCMouseMove(UINT flags, const gfx::Point& point)
    {
        // NC points are in screen coordinates.
        POINT temp = point.ToPOINT();
        MapWindowPoints(HWND_DESKTOP, hwnd(), &temp, 1);
        ProcessMouseMoved(gfx::Point(temp), 0, true);
    ...
    void WidgetWin::OnNCRButtonDblClk(UINT flags, const gfx::Point& point)
    {
        SetMsgHandled(ProcessMousePressed(point, flags|MK_RBUTTON, true, true));
    }

    void WidgetWin::OnNCRButtonDown(UINT flags, const gfx::Point& point)
    {
        SetMsgHandled(ProcessMousePressed(point, flags|MK_RBUTTON, false, true));
    }

    void WidgetWin::OnNCRButtonUp(UINT flags, const gfx::Point& point)
    {
        SetMsgHandled(FALSE);
    }
    ...
    void WidgetWin::OnRButtonDown(UINT flags, const gfx::Point& point)
    {
        ProcessMousePressed(point, flags|MK_RBUTTON, false, false);
    }

    void WidgetWin::OnRButtonUp(UINT flags, const gfx::Point& point)
    {
        ProcessMouseReleased(point, flags|MK_RBUTTON);
    }

    void WidgetWin::OnRButtonDblClk(UINT flags, const gfx::Point& point)
    {
    ...
    {
        GetFocusManager()->GetWidgetFocusManager()->OnWidgetFocusEvent(
            focused_window, this->GetNativeView());
        SetMsgHandled(FALSE);
    }
    ...
    bool WidgetWin::ProcessMousePressed(const gfx::Point& point,
    ...
    void WidgetWin::ProcessMouseDragged(const gfx::Point& point, UINT flags)
    {
        last_mouse_event_was_move_ = false;
    ...
    void WidgetWin::ProcessMouseReleased(const gfx::Point& point, UINT flags)
    {
        last_mouse_event_was_move_ = false;
    ...
    void WidgetWin::ProcessMouseMoved(const gfx::Point &point, UINT flags,
        bool is_nonclient)
    {
    ...
    void WidgetWin::UpdateWindowFromContents(HDC dib_dc)
    {
        ...
        if(can_update_layered_window_)
        {
            ...
            POINT zero_origin = { 0, 0 };
            POINT window_position = { wr.left, wr.top };

            BLENDFUNCTION blend = {AC_SRC_OVER, 0, layered_alpha_, AC_SRC_ALPHA};
            UpdateLayeredWindow(hwnd(), NULL, &window_position, &size, dib_dc,
                &zero_origin, RGB(0xFF, 0xFF, 0xFF), &blend, ULW_ALPHA);
        }
    }
    ...
    {
        RootView* root_view = reinterpret_cast<RootView*>(
            GetProp(hwnd, kRootViewWindowProperty));
        if(root_view)
        {
    ...
    {
        RootView* root_view = reinterpret_cast<RootView*>(
            GetProp(window, kRootViewWindowProperty));
        std::set<RootView*> root_views_set;
        if(root_view)
        {
            root_views_set.insert(root_view);
    ...
    // static
    Widget* Widget::GetWidgetFromNativeView(HWND native_view)
    {
    ...
    // static
    Widget* Widget::GetWidgetFromNativeWindow(HWND native_window)
    {
