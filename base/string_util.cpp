...
{
    return base::Singleton<EmptyStrings>::get()->s;
}

const std::wstring& EmptyWString()
{
    return base::Singleton<EmptyStrings>::get()->ws;
}

const string16& EmptyString16()
{
    return base::Singleton<EmptyStrings>::get()->s16;
}
...
{
    // 显示单位的字节数阀值.
    // 显示单位U满足: kUnitThresholds[U] <= bytes < kUnitThresholds[U+1].
