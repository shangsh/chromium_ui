...
: data_(data), language_(language),
code_page_(code_page), fixed_file_info_(NULL)
{
    UINT size;
    VerQueryValueW(data_, L"\\", (LPVOID*)&fixed_file_info_, &size);
}
...
// static
FileVersionInfo* FileVersionInfo::CreateFileVersionInfo(const base::FilePath& file_path)
{
    DWORD dummy;
    DWORD length = GetFileVersionInfoSizeW(file_path.value(), &dummy);
    if(length == 0)
    {
        return NULL;
    }
    ...
    if(!GetFileVersionInfoW(file_path.value(), dummy, length, data))
    {
        free(data);
        return NULL;
    }
    ...
    BOOL query_result = VerQueryValueW(data, L"\\VarFileInfo\\Translation",
        (void**)&translate, &page_count);

    if(query_result && translate)
    {
        return new FileVersionInfo(data, translate->language, translate->code_page);
    }
    else
    {
        free(data);
