...
namespace gfx
{
    ...
    // static
    PlatformFontWin::HFontRef* PlatformFontWin::base_font_ref_ = NULL;
    ...
    {
        return length * __min(font_ref_->dlu_base_x(), GetAverageCharacterWidth());
    }
    ...
    PlatformFontWin::HFontRef* PlatformFontWin::CreateHFontRef(HFONT font)
    {
        ...
        const int height = __max(1, static_cast<int>(font_metrics.tmHeight));
        const int baseline = __max(1, static_cast<int>(font_metrics.tmAscent));
        const int ave_char_width =
            __max(1, static_cast<int>(font_metrics.tmAveCharWidth));
        int style = 0;
